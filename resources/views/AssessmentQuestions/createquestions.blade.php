    ...
    <div class="max-w-3xl mx-auto p-8 bg-white shadow rounded-xl mb-20">
        ...
        <div class="flex items-center gap-2 mb-8">
            ...
            <h1 class="text-2xl font-bold text-black">
                Add Questions 
                <span class="text-sm text-black">(10 per Level – Auto Assigned)</span>
            </h1>
        </div>
        ...
        <form action="{{ route('Questions.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            ...
            <div>
                ...
                <select name="position" class="w-full border rounded-lg p-2 mt-1 focus:ring-2 focus:ring-green-400 text-black" required>
            ...
            <div>
                ...
                <select name="assessment_id" class="w-full border rounded-lg p-2 mt-1 focus:ring-2 focus:ring-green-400 text-black" required>
            ...
            <div>
                ...
                <div class="mt-2 flex items-center gap-3 p-3 border-2 border-dashed rounded-lg hover:border-green-500 cursor-pointer">
                    ...
                    <input type="file" id="questionFile" class="flex-1 text-sm text-black" accept=".jpg,.jpeg,.png,.doc,.docx">
                </div>
                <small class="text-black">Supported: JPG, PNG, DOC, DOCX</small>
            </div>
    ...
    <script>
        ...
        function renderQuestions(questions) {
            ...
            questions.forEach((q, i) => {
                ...
                block.innerHTML = `
                    <label class="flex items-center gap-2 mb-2 font-semibold text-black">
                        <i data-lucide="help-circle" class="w-5 h-5 text-green-600"></i>
                        Q${i+1}
                    </label>
                    <textarea class="border rounded-lg p-2 w-full focus:ring-2 focus:ring-green-400 text-black" name="questions[${i}][question]">${q.question}</textarea>

                    ${['A','B','C','D'].map(opt => `
                        <label class="block mt-2 text-black">Option ${opt}
                            <input type="text" class="border rounded-lg p-1 w-full mt-1 focus:ring-2 focus:ring-green-400 text-black" 
                                   name="questions[${i}][option_${opt.toLowerCase()}]" value="${q.options[opt] || ""}">
                        </label>
                    `).join('')}

                    <label class="block mt-3 font-semibold text-black">Correct Answer
                        <input type="text" class="border rounded-lg p-1 w-full mt-1 bg-green-100 focus:ring-2 focus:ring-green-400 text-black" 
