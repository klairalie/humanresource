<x-guest-layout>
    <div class="p-6">
        <h1 class="text-xl font-bold mb-4">Add Questions (10 per Level – Auto Assigned)</h1>
        ...
        <form action="{{ route('Questions.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
            ...
            <label class="block mt-4">
                ...
                <select name="assessment_id" class="w-full border rounded p-2" required>
            ...
            <!-- Upload -->
            <label class="block mt-4">
                <span>Upload Questionnaire (Image / DOCX)</span>
                <input type="file" id="questionFile" class="w-full border rounded p-2" accept=".jpg,.jpeg,.png,.doc,.docx">
                <small class="text-gray-600">Supported: JPG, PNG, DOC, DOCX</small>
            </label>

            <!-- Auto-Filled Questions Container -->
            <div id="questionsContainer" class="space-y-6 mt-4"></div>

            <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded mb-15">Save</button>
        </form>
    </div>
