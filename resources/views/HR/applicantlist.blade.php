<x-guest-layout>
    <div class="min-h-screen p-6 mt-10" x-data="applicantModal()" x-cloak>
        <!-- Header Section -->
...
<td class="px-4 py-2 w-56">
    @php
        ...
        $statusClasses = [
            ...
            'On Screening'        => 'bg-blue-100 text-blue-800',
            'Passed Screening'    => 'bg-yellow-100 text-yellow-800',
            'Reviewed'            => 'bg-indigo-100 text-indigo-800',
            'Scheduled Interview' => 'bg-purple-100 text-purple-800',
            'Failed Screening'    => 'bg-red-100 text-red-800',
            'Done'                => 'bg-green-600 text-white',
            'Unattended'          => 'bg-gray-300 text-red-500',
        ...
        // Check if applicant has results
        $hasAssessment = \DB::table('assessment_results')
            ->where('applicant_id', $applicant->applicant_id)
            ->exists();

        // Temporary override only if still On Screening
        if ($status === 'On Screening' && $hasAssessment) {
            $status = 'Done Taking Assessment';
            $classApplicant = 'bg-green-100 text-green-800';
        } else {
            $classApplicant = $statusClasses[$status] ?? 'bg-gray-200 text-gray-800';
        }

        $interviewStatus = ($status !== 'Hired' && $status !== 'Rejected') ? ($interview->status ?? null) : null;
        $classInterview = $interviewStatus ? ($statusClasses[$interviewStatus] ?? 'bg-gray-200 text-gray-800') : null;
    @endphp
...
<td class="px-4 py-2">
    <div class="flex flex-wrap justify-center gap-2">
        ...
        @else
            {{-- Summarize button if assessment exists and applicant still on screening --}}
            @if ($applicant->applicant_status === 'On Screening' && $hasAssessment)
                <form action="{{ route('applicants.summarize', $applicant->applicant_id) }}" method="POST">
                    @csrf
                    <button type="submit" class="px-3 py-1 bg-teal-600 text-white rounded hover:bg-teal-700">
                        Summarize
                    </button>
                </form>
            @elseif ($applicant->applicant_status === 'Passed Screening')
                <a href="{{ route('review.document', $applicant->applicant_id) }}" class="px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700">
                    Review Document
                </a>
        ...
        <div x-show="show" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" x-cloak>
            <div class="bg-white w-full max-w-3xl p-6 rounded-lg shadow-lg overflow-y-auto max-h-[100vh] relative">
                ...
                <div class="space-y-3 text-left">
                    ...
                    <ul class="list-disc pl-6">
                        <li class="mb-1"><a :href="'/storage/' + applicant.resume_file" target="_blank" class="text-blue-600 hover:underline">Resume</a></li>
                        <li class="mb-1"><a :href="'/storage/' + applicant.good_moral_file" target="_blank" class="text-blue-600 hover:underline">Good Moral</a></li>
                        <li class="mb-1"><a :href="'/storage/' + applicant.coe_file" target="_blank" class="text-blue-600 hover:underline">Certificate of Employment</a></li>
                    </ul>
                </div>
                ...
                <div class="mt-6 flex flex-col md:flex-row md:items-center md:justify-end space-y-2 md:space-y-0 md:space-x-3">
                    <form :action="`/assessment/${applicant.id}/${selectedAssessment}`" method="POST" class="flex items-center space-x-2">
                        ...
                        <label>
                            ...
                            <select x-model="selectedAssessment" class="border rounded p-1 ml-2 w-sm" required>
                        ...
                        <button type="submit" class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 ml-2 mt-5">Send </button>
                    </form>
                    ...
                    <button @click="closeModal" class="px-3 py-1 bg-gray-600 text-white rounded hover:bg-gray-700 mt-5">Close</button>
                </div>
            </div>
        </div>
    </div>
