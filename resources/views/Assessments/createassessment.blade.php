{{-- CREATE ASSESSMENT FORM FOR THE TOKEN {{ $tokenRecord->token }} --}}

<x-guest-layout>
    <div class="max-w-2xl mx-auto p-6 bg-white shadow rounded-lg mt-30">
        <h1 class="text-2xl font-bold mb-6 text-black">Add Assessment</h1>
        ...
        <form action="{{ route('assessments.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select 
                    id="title" 
                    name="title" 
                    class="border border-gray-300 rounded-lg w-full p-2 focus:ring focus:ring-green-300 focus:outline-none text-black" 
                    required
            ...
            <div id="positionWrapper">
                <label for="position_name" class="block text-sm font-medium text-black mb-1">
                    Position <span id="positionRequired" class="text-red-500">*</span>
                </label>
                <select 
                    id="position_name" 
                    name="position_name" 
                ...
                <small id="positionNote" class="text-black">Required for Applicant Tests, optional for Evaluations</small>
            </div>
            ...
            <div class="pt-2 flex gap-3">
                <a href="{{ url()->previous() }}" 
                   class="px-5 py-2 bg-gray-300 hover:bg-gray-400 text-black font-medium rounded-lg shadow">
                    ← Back
                </a>
                <button 
                    type="submit" 
    ...
    <script>
        function updateTypeAndPosition(select) {
            ...
            let positionNote = document.getElementById("positionNote");
            let positionRequired = document.getElementById("positionRequired");

            positionWrapper.style.display = "block"; // always show for both

            if (type === "test") {
                positionSelect.setAttribute("required", "required");
                positionRequired.style.display = "inline"; 
                positionNote.textContent = "Required for Applicant Tests";
            } else if (type === "evaluation") {
                positionSelect.removeAttribute("required");
                positionRequired.style.display = "none"; 
                positionNote.textContent = "Optional for Evaluations";
            }
